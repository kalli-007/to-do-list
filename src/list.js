...
return (
    <div className="p-5 flex items-center justify-center flex-col gap-4">
        ...
        <div className="w-full max-w-md" style={{ width: "350px" }}>
            <ul className="flex flex-col h-64 overflow-y-auto">
                {todos.map((todo, index) => (
                    <li
                        key={index}
                        className="bg-white p-3 rounded-md mb-2 hover:bg-violet-300 transition duration-300  flex justify-between "
                    >
                        <span
